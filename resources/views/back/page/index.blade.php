          ...
          <div class="row">
            <div class="col-md-12">
              <div class="block-flat">
                <div class="content">
                  <a href="{{ url('/manage/page/create') }}" class="btn btn-primary">Thêm mới</a>
                              ...
                              @foreach($pages as $page)
                                <tr class="" role="">
                                    ...
                                    <td>
                                      ...
                                      @else
                                        <img src="{{ config('image.image_url').'/pages/'.$page->thumbnail.'_70x70.png' }}" />
                                      @endif
                                    </td>
                                    <td>{!! $page->title !!}</td>
                                    <td>{!! $page->content !!}</td>
                                    <td class="center">{{ date('d-m-Y H:i:s', strtotime($page->created_at)) }}</td>
                                    <td><a class="btn btn-primary btn-xs" href="{{ url('/manage/page/'.$page->id) }}"  data-toggle="tooltip"><i class="fa fa-pencil"></i></a>
                                        <button data-toggle="modal" data-target="#mod-error" class="delete_page btn btn-danger btn-xs" page-id="{{ $page->id }}"  ><i class="fa fa-times"></i></button>
                                     </td>
                                </tr>
                                <?php $i++?>
                              @endforeach
      ...
      $(document).ready(function(){
        var dataTable = $('#datatable-icons').DataTable( {
          ...
          $('.delete_page').click(function() {          
            ...
            $('#form_model').attr('action', '/manage/page/'+id+'/destroy');
          }); 
