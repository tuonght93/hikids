...
@section('title')
   <title>City | HappySkin Admin</title>
@stop
          ...
          <div class="row">
            <div class="col-md-12">
              <div class="block-flat">
                <div class="content">
                  <a href="{{ url('/manage/city/create') }}" class="btn btn-primary">Thêm mới</a>
                          ...
                          <table id="datatable-icons" class="table table-bordered dataTable no-footer" role="grid" aria-describedby="datatable-icons_info">
                            <thead>
                              <tr role="row">
                                ...
                                <th style="width: 10%;">Miễn phí giao hàng</th>
                                <th style="width: 10%;">Phi giao hàng</th>
                            ...
                            @if ($citys)
                              ...
                              @foreach($citys as $city)
                                <tr class="" role="">
                                    ...
                                    <td>{!! $city->name !!}</td>
                                    <td>{!! $city->slug !!}</td>
                                    <td>{!! number_format($city->freeshipping,0,",",".") !!}đ</td>
                                    <td>{!! number_format($city->shippingcost,0,",",".") !!}đ</td>
                                    <td class="center">{{ date('d-m-Y H:i:s', strtotime($city->created_at)) }}</td>
                                    <td><a class="btn btn-primary btn-xs" href="{{ url('/manage/city/'.$city->id) }}"  data-toggle="tooltip"><i class="fa fa-pencil"></i></a>
                                        <button data-toggle="modal" data-target="#mod-error" class="delete_city btn btn-danger btn-xs" city-id="{{ $city->id }}"  ><i class="fa fa-times"></i></button>
                                     </td>
                                </tr>
                                <?php $i++?>
                              @endforeach
      ...
      $(document).ready(function(){
        var dataTable = $('#datatable-icons').DataTable( {
          ...
          $('.delete_city').click(function() {          
            ...
            $('#form_model').attr('action', '/manage/city/'+id+'/destroy');
          }); 
