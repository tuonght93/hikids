...
@section('title')
   <title>District | Hikids Admin</title>
...
@section('content')
      <div class="page-head">
        <h2>{{ empty($district->id) ? 'Thêm mới quận huyện' : 'Sửa quận huyện' }}</h2>
        <ol class="breadcrumb">
          <li><a href="{{ url('/manage/district') }}">District</a></li>
          <li>{{ empty($district->id) ? 'Thêm mới' : 'Sửa' }}</li>
        </ol>
      </div>
      <div class="cl-mcont">
          <div class="row">
            <div class="col-md-12">            
            {!! Form::open(['url' => '/manage/district/'.$district->id, 'method' => empty($district->id) ? 'POST' : 'PUT', 'role' => 'form', 'files' => 'true', 'class' => 'form-horizontal group-border-dashed', 'style' => 'border-radius: 0px;', 'id' => 'form_district',]) !!} 
                ...
                <div class="content">
                  ...
                  @include('errors/error_validation', ['errors' => $errors])
                    ...
                    <div class="form-group">
                      <label class="col-sm-3 control-label">Tên quận/huyện</label>
                      <div class="col-sm-6">
                        {!! Form::text('name', $district->name, array('placeholder' => 'Tiêu đề', 'class' => 'form-control')) !!}
                      </div>
                    </div>
                    ...
                    <div class="form-group">
                      ...
                      <div class="col-sm-6">
                        ...
                        @foreach($cities as $city)
                          ...
                          ?>             
                        @endforeach
                        {!! Form::select('city', $data_cities,$district->city_id, array('id' => 'city', 'class' => 'form-control')) !!}
                       <!--<input type="hidden" name="categories" id="categories"/>-->
                      </div>
                    </div>

                    <div class="form-group">
                      <label class="col-sm-3 control-label">Giá chuyển hàng nhanh (nếu có)</label>
                      <div class="col-sm-6">
                        {!! Form::text('shippingfast', $district->shippingfast, array('placeholder' => 'Giá chuyển hàng nhanh', 'class' => 'form-control')) !!}
                      </div>
                    </div>
                </div>
              </div>  
              ...
              <div class="row block-flat">
                <div class="col-sm-offset-2 col-sm-10">
                    <a href="/manage/district" class="btn btn-default">Trở lại</a>
    ...
    <script type="text/javascript">
      $(document).ready(function(){
        ...
        $('#form_submit').click(function(){
          // $("#content").val($("#district_content").code());
