...
@section('title')
   <title>District | HappySkin Admin</title>
@stop
          ...
          <div class="row">
            <div class="col-md-12">
              <div class="block-flat">
                <div class="content">
                  <a href="{{ url('/manage/district/create') }}" class="btn btn-primary">Thêm mới</a>
                          ...
                          <table id="datatable-icons" class="table table-bordered dataTable no-footer" role="grid" aria-describedby="datatable-icons_info">
                            <thead>
                              <tr role="row">
                                ...
                                <th style="width: 10%;">Giá chuyển hàng nhanh</th>
                              ...
                              @foreach($districts as $district)
                                <tr class="" role="">
                                    ...
                                    <td>
                                      ...
                                      {!! number_format($district->shippingfast,0,",",".") !!}đ
                                      @endif
                                    </td>
                                    <td class="center">{{ date('d-m-Y H:i:s', strtotime($district->created_at)) }}</td>
                                    <td><a class="btn btn-primary btn-xs" href="{{ url('/manage/district/'.$district->id) }}"  data-toggle="tooltip"><i class="fa fa-pencil"></i></a>
                                        <button data-toggle="modal" data-target="#mod-error" class="delete_district btn btn-danger btn-xs" district-id="{{ $district->id }}"  ><i class="fa fa-times"></i></button>
                                     </td>
                                </tr>
                                <?php $i++?>
                              @endforeach
      ...
      $(document).ready(function(){
        var dataTable = $('#datatable-icons').DataTable( {
          ...
          $('.delete_district').click(function() {          
            var id = $(this).attr('district-id');
            $('#form_model').attr('action', '/manage/district/'+id+'/destroy');
          }); 
