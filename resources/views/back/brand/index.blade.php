          ...
          <div class="row">
            <div class="col-md-12">
              <div class="block-flat">
                <div class="content">
                  <a href="{{ url('/manage/brand/create') }}" class="btn btn-primary">Thêm mới</a>
                              ...
                              @foreach($brands as $brand)
                                <tr class="" role="">
                                    ...
                                    <td>
                                      ...
                                      @else
                                        <img src="{{ config('image.image_url').'/brands/'.$brand->thumbnail.'_70x70.png' }}" />
                                      @endif
                                    </td>
                                    <td>{!! $brand->title !!}</td>
                                    <td>{!! $brand->content !!}</td>
                                    <td class="center">{{ date('d-m-Y H:i:s', strtotime($brand->created_at)) }}</td>
                                    <td><a class="btn btn-primary btn-xs" href="{{ url('/manage/brand/'.$brand->id) }}"  data-toggle="tooltip"><i class="fa fa-pencil"></i></a>
                                        <button data-toggle="modal" data-target="#mod-error" class="delete_brand btn btn-danger btn-xs" brand-id="{{ $brand->id }}"  ><i class="fa fa-times"></i></button>
                                     </td>
                                </tr>
                                <?php $i++?>
                              @endforeach
      ...
      $(document).ready(function(){
        var dataTable = $('#datatable-icons').DataTable( {
          ...
          $('.delete_brand').click(function() {          
            ...
            $('#form_model').attr('action', '/manage/brand/'+id+'/destroy');
          }); 
