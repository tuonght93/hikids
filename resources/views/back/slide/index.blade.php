          ...
          <div class="row">
            <div class="col-md-12">
              <div class="block-flat">
                <div class="content">
                  <a href="{{ url('/manage/slide/create') }}" class="btn btn-primary">Thêm mới</a>
                              ...
                              @foreach($slides as $slide)
                                <tr class="" role="">
                                    ...
                                    <td>
                                      ...
                                      @else
                                        <img src="{{ config('image.image_url').'/slides/'.$slide->thumbnail.'_70x70.png' }}" />
                                      @endif
                                    </td>
                                    <td>{!! $slide->link !!}</td>
                                    <td>{!! $slide->title !!}</td>
                                    <td>{!! $slide->content !!}</td>
                                    <td class="center">{{ date('d-m-Y H:i:s', strtotime($slide->created_at)) }}</td>
                                    <td><a class="btn btn-primary btn-xs" href="{{ url('/manage/slide/'.$slide->id) }}"  data-toggle="tooltip"><i class="fa fa-pencil"></i></a>
                                        <button data-toggle="modal" data-target="#mod-error" class="delete_slide btn btn-danger btn-xs" slide-id="{{ $slide->id }}"  ><i class="fa fa-times"></i></button>
                                     </td>
                                </tr>
                                <?php $i++?>
                              @endforeach
      ...
      $(document).ready(function(){
        var dataTable = $('#datatable-icons').DataTable( {
          ...
          $('.delete_slide').click(function() {          
            ...
            $('#form_model').attr('action', '/manage/slide/'+id+'/destroy');
          }); 
