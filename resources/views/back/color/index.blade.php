          ...
          <div class="row">
            <div class="col-md-12">
              <div class="block-flat">
                <div class="content">
                  <a href="{{ url('/manage/color/create') }}" class="btn btn-primary">Thêm mới</a>
                              ...
                              @foreach($colors as $color)
                                <tr class="" role="">
                                    ...
                                    <td>
                                      ...
                                      @else
                                        <img src="{{ config('image.image_url').'/colors/'.$color->thumbnail.'_70x70.png' }}" />
                                      @endif
                                    </td>
                                    <td>{!! $color->title !!}</td>
                                    <td>{!! $color->content !!}</td>
                                    <td class="center">{{ date('d-m-Y H:i:s', strtotime($color->created_at)) }}</td>
                                    <td><a class="btn btn-primary btn-xs" href="{{ url('/manage/color/'.$color->id) }}"  data-toggle="tooltip"><i class="fa fa-pencil"></i></a>
                                        <button data-toggle="modal" data-target="#mod-error" class="delete_color btn btn-danger btn-xs" color-id="{{ $color->id }}"  ><i class="fa fa-times"></i></button>
                                     </td>
                                </tr>
                                <?php $i++?>
                              @endforeach
      ...
      $(document).ready(function(){
        var dataTable = $('#datatable-icons').DataTable( {
          ...
          $('.delete_color').click(function() {          
            ...
            $('#form_model').attr('action', '/manage/color/'+id+'/destroy');
          }); 
